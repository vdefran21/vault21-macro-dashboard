...
import { fmtDateTime, fmtShortDate } from '../../lib/formatters';
...
function TimelineTooltip({ active, payload }) {
  ...
  return (
    ...
    >
      <div style={{ color: COLORS.amber, marginBottom: 4 }}>{fmtDateTime(d?.date, d?.event_time)}</div>
    ...
    <Card title="Crisis Escalation Timeline" subtitle="Event severity (1-6 scale) — acceleration pattern visible from Feb 2026 onward">
      <ResponsiveContainer width="100%" height={280}>
        <ComposedChart data={data}>
          ...
          <XAxis
            dataKey="chart_key" tick={{ ...AXIS_TICK, fontSize: 9 }}
            tickFormatter={(value) => fmtShortDate(String(value).split('__')[0])}
